...
/** Proxy Nest GET/POST /api/v1/admin/hotel-memberships */
export async function GET(req: Request) {
    try {
        ...
        if (!hotelId) {
            return NextResponse.json(
                { error: 'hotelId query param is required' },
                { status: 400 },
            );
        }
        ...
        const url = new URL(`${backendUrl}/api/v1/admin/hotel-memberships`);
        ...
        console.error('Admin hotel-memberships GET proxy error', e);
...
export async function POST(req: Request) {
    try {
        ...
        const response = await fetch(
            `${backendUrl}/api/v1/admin/hotel-memberships`,
            {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
            ...
            return NextResponse.json(
                {
                    error:
                        (data as { message?: string }).message ||
                        'Failed to assign hotel membership',
                },
                { status: response.status },
            );
        }
        ...
        console.error('Admin hotel-memberships POST proxy error', e);
        return NextResponse.json({ error: 'Internal error' }, { status: 500 });
    }
}
