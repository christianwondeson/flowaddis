...
export const HotelDetailSidebar: React.FC<HotelDetailSidebarProps> = ({
    hotel,
    reviews = [],
    loading = false,
    onBook,
        ...
        <div className="space-y-4">
                ...
                {loading ? (
                    <div className="animate-pulse space-y-4">
                        <div className="flex justify-between items-center">
                            ...
                            <div className="h-10 w-10 bg-gray-200 dark:bg-slate-700 rounded-t-xl rounded-br-xl" />
                        </div>
                        <div className="h-12 bg-gray-100 dark:bg-slate-800 rounded w-full" />
                    </div>
                ) : reviews.length > 0 ? (
                    <>
                        <div className="flex items-center justify-between mb-4">
                            <div className="text-right">
                                <div className="font-bold text-foreground">{ratingWord}</div>
                                <div className="text-[10px] text-muted-foreground">
                                    {t('hotelDetail.sidebar.reviewsShort', {
                                        count: hotel.reviews ?? reviews.length,
                                    })}
                                </div>
                            </div>
                            <div className="w-10 h-10 bg-brand-primary text-white rounded-t-xl rounded-br-xl flex items-center justify-center font-bold text-lg shadow-sm shadow-brand-primary/20">
                                {Number(hotel.rating || reviews[0]?.average_score || 0).toFixed(1)}
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            <div className="text-[11px] text-muted-foreground italic leading-relaxed border-l-2 border-brand-primary/20 pl-3 line-clamp-3">
                                ...
                                {reviews[0].pros ||
                                    reviews[0].title ||
                                    t('hotelDetail.sidebar.noComment')}
                                &rdquo;
                            </div>
                    ...
                    <div className="text-center py-4">
                        <div className="text-sm font-bold text-foreground mb-1">
                            {t('hotelDetail.sidebar.noReviewsYet')}
                        </div>
                        <div className="text-[10px] text-muted-foreground">
                            {t('hotelDetail.sidebar.beFirstToReview')}
                        </div>
                    </div>
                )}
            </div>
            ...
            <div className="bg-brand-primary/5 dark:bg-brand-primary/10 border border-brand-primary/10 dark:border-brand-primary/20 rounded-xl p-4 space-y-4">
                <h4 className="font-bold text-foreground text-sm">
                    {t('hotelDetail.sidebar.propertyHighlights')}
                </h4>
                <div className="space-y-3">
                    ...
                    <p className="text-[11px] text-muted-foreground">
                        {hotel.checkin
                            ? `${t('hotelDetail.checkIn')} ${hotel.checkin}`
                            : t('hotelDetail.sidebar.noChargeYet')}
                    </p>
                </div>

                <Button
                    onClick={() => onBook?.()}
                    className="w-full bg-brand-primary hover:bg-brand-primary/90 text-white font-bold text-xs py-2 rounded-lg shadow-md shadow-brand-primary/10 transition-all active:scale-95"
                >
                ...
                <Button
                    asChild
                    variant="outline"
                    className="w-full font-bold text-xs py-2 rounded-lg gap-2"
                >
                    <Link href={`/hotels/${hotel?.id}/messages`}>
